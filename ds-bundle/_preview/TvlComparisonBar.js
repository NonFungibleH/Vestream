...
var __dsPreview = (() => {
  ...
  // .design-sync/previews/TvlComparisonBar.tsx
  var TvlComparisonBar_exports = {};
  __export(TvlComparisonBar_exports, {
    Default: () => Default
  });
  init_define_import_meta_env();
  ...
  var CHAIN_IDS = {
    ...
    BASE: 8453,
    ARBITRUM: 42161,
    // Arbitrum One — largest L2 by TVL
    OPTIMISM: 10,
    // OP Mainnet — major L2 (OP token vesting + ecosystem)
    AVALANCHE: 43114,
    // Avalanche C-Chain — vesting TVL across Sablier/Hedgey/Team Finance/LlamaPay
    SEPOLIA: 11155111,
    // Ethereum Sepolia testnet
    BASE_SEPOLIA: 84532,
    // Base Sepolia testnet
    SOLANA: 101
    // Solana mainnet-beta (non-EVM)
  };
  var CHAIN_NAMES = {
  ...
  var PROTOCOLS = {
    ...
    hedgey: {
      ...
      description: "Hedgey represents every vesting plan as an NFT – portable, on-chain, and fully visible in any wallet. It's the default for team token distribution at dozens of mid-cap projects. Vestream indexes every Hedgey plan across Ethereum, Base, BSC, Polygon and Arbitrum and reminds the beneficiary before each unlock.",
      color: "#33406B",
    ...
    unvest: {
      ...
      description: "Unvest gives token issuers fine-grained control over release schedules – step-by-step milestones, custom cliffs, and per-beneficiary plans. Common for token sales that need deliberate, event-driven release. Vestream indexes every Unvest V3 contract across Ethereum, Base, BSC, Polygon, Arbitrum, and Optimism.",
      color: "#2563EB",
      bg: "rgba(37,99,235,0.08)",
    ...
    pinksale: {
      ...
      description: "PinkSale is the launchpad many token projects used for their initial offering – and PinkLock is its on-chain token locker for team allocations and LP proofs. Vestream reads every PinkLock contract directly (no subgraph) across Ethereum, Base, BSC and Polygon and lines up each TGE + cycle release in one view.",
      color: "#F23E8C",
    ...
    streamflow: {
      ...
      description: "Streamflow is the dominant token-vesting protocol on Solana – the go-to rail for SPL token launches, team vesting and investor unlocks. Vestream indexes Streamflow alongside the EVM ecosystem so cross-chain holders see every unlock – Ethereum, BSC, Polygon, Base and Solana – in one dashboard.",
      color: "#2F54EB",
      // Streamflow blue
      ...
      externalTvl: {
        ...
        slug: "streamflow",
        category: "vesting"
        // excludes DefiLlama's "Payments" TVL (~$500k)
    ...
    "jupiter-lock": {
      ...
      description: "Jupiter Lock is the default token-vesting rail in the Jupiter ecosystem – used by JUP's own team-and-investor allocations and the majority of Solana launchpad deals since late 2024. Vestream reads every active VestingEscrow directly from the program so recipients see their cliff, periodic drip, and claimable amount in one view – no need to open the Jupiter UI.",
      color: "#14B8A6",
      // Jupiter teal
  ...
  var PROTOCOL_BRAND = (() => {
    ...
    if (m["uncx"]) m["uncx-vm"] = m["uncx"];
    return m;
  })();
  var PROTOCOL_BRAND_FALLBACK = {
    color: "#64748b",
    bg: "rgba(148,163,184,0.10)",
    border: "rgba(148,163,184,0.20)",
    name: ""
  };
  function protocolBrand(slug) {
    return PROTOCOL_BRAND[slug] ?? { ...PROTOCOL_BRAND_FALLBACK, name: slug };
  }
  function protocolChip(slug) {
    const b = protocolBrand(slug);
    return { text: b.color, bg: b.bg, border: b.border };
  }
  var PROTOCOL_CHIPS = Object.fromEntries(Object.keys(PROTOCOL_BRAND).map((s) => [s, protocolChip(s)]));
  var PINKSALE_CONTRACT_ADDRESSES = {
  ...
  var tvl = (usd, high, medium, low, priced, skipped, chains) => ({
    adapterIds: [],
    tvlUsd: usd,
    tvlByBand: { high, medium, low },
    pricingSources: { dexscreener: Math.round(priced * 0.8), defillama: 0, coingecko: Math.round(priced * 0.2) },
    perChain: chains.map(([chainId, tvlUsd]) => ({ chainId, tvlUsd })),
    tokensPriced: priced,
    tokensSkipped: skipped,
    totalTokens: priced + skipped,
    coverage: priced / (priced + skipped),
    topContributors: [],
    computedAt: "2024-05-15T06:00:00.000Z"
  });
  var ROWS = [
    { protocol: getProtocol("streamflow"), tvl: tvl(762e6, 75e7, 12e6, 0, 48, 6, [[101, 762e6]]), activeStreams: 5110, indexedChainCount: 1 },
    { protocol: getProtocol("sablier"), tvl: tvl(513e6, 5e8, 13e6, 0, 132, 18, [[1, 38e7], [8453, 96e6], [42161, 37e6]]), activeStreams: 8420, indexedChainCount: 5 },
    { protocol: getProtocol("hedgey"), tvl: tvl(142e6, 138e6, 4e6, 0, 61, 9, [[1, 104e6], [8453, 38e6]]), activeStreams: 2310, indexedChainCount: 6 },
    { protocol: getProtocol("uncx"), tvl: tvl(485e5, 4e7, 85e5, 0, 74, 21, [[56, 31e6], [1, 175e5]]), activeStreams: 1290, indexedChainCount: 4 }
  ].filter((r) => r.protocol);
  var Default = () => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
    ds_exports.TvlComparisonBar,
    {
      rows: ROWS,
      externallySourced: /* @__PURE__ */ new Set(["sablier", "streamflow", "hedgey"]),
      snapshotAgeHours: 6
    }
  );
  return __toCommonJS(TvlComparisonBar_exports);
})();
