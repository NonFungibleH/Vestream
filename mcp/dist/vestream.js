...
// ─── API helper ───────────────────────────────────────────────────────────────
async function vstreamFetch(path) {
...
server.tool("get_wallet_vestings", "Get all token vesting streams for an EVM wallet address across all supported " +
    ...
    address: z.string().describe("EVM wallet address in 0x format, e.g. '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045'"),
    protocol: z.string().optional().describe("Comma-separated protocol filter, e.g. 'sablier,uncx'. " +
        "Valid values: sablier, uncx, hedgey, team-finance, unvest"),
    chain: z.string().optional().describe("Comma-separated chain ID filter, e.g. '1,8453'. " +
        "Supported: 1 (Ethereum), 56 (BSC), 8453 (Base)"),
    active_only: z.boolean().optional().describe("If true, only return streams that are not yet fully vested (default: false)"),
...
server.tool("get_upcoming_unlocks", "Get all upcoming token unlock events for an EVM wallet within a future time window. " +
    ...
    address: z.string().describe("EVM wallet address in 0x format"),
    days: z.number().optional().describe("Lookahead window in days (default: 30, max: 365)"),
    protocol: z.string().optional().describe("Comma-separated protocol filter, e.g. 'sablier,uncx'"),
...
// ── Tool: get_stream ──────────────────────────────────────────────────────────
server.tool("get_stream", "Get full details for a single vesting stream by its composite ID. " +
    ...
    stream_id: z.string().describe("Composite stream ID in format 'protocol-chainId-nativeId', " +
...
//# sourceMappingURL=vestream.js.map