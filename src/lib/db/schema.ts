...
export const users = pgTable("users", {
  ...
  // Free-tier push credits (10 per calendar month, resets on the 1st).
  // Was originally a 3-lifetime cap — flipped to monthly reset May 2026
  // to make the free tier actually usable. pushAlertsSent is the in-month
  // counter; pushAlertsMonthStart records which month it belongs to.
  // checkAndConsumePushCredit rolls the counter back to 0 + bumps this
...
// ── Unlock events (accrual-basis tax) ───────────────────────────────────────
// Mirrors claimEvents, but for the moment tokens *vest* rather than the moment
// the user *claims* them. Unlocks are NOT on-chain events — there's no tx to
// index — so these rows are COMPUTED from each stream's vesting schedule (see
// computeUnlockTranches in lib/vesting/unlock-schedule.ts), then priced at the
// unlock timestamp. This is the income half of the tax tool for accrual-basis
// jurisdictions (tax due when tokens become available, not when withdrawn).
// No tx/gas columns (there is no transaction); adds a manual-FMV flag so a user
// can hand-enter a price when we can't source one.
export const vestingUnlockEvents = pgTable("vesting_unlock_events", {
  ...
  protocol: text("protocol").notNull(),
  chainId: integer("chain_id").notNull(),
  /** Recipient wallet (canonical lowercase EVM / preserved Solana base58). */
  recipient: text("recipient").notNull(),
  /** Token contract (canonical lowercase EVM / preserved Solana). */
  tokenAddress: text("token_address").notNull(),
  tokenSymbol: text("token_symbol"),
  tokenDecimals: integer("token_decimals").notNull(),
  /** Amount unlocked in this tranche, base units (stringified bigint). */
  amount: text("amount").notNull(),
  /** When the tranche vested / became available. */
  unlockTime: timestamp("unlock_time").notNull(),
  /** Stable dedup key from computeUnlockTranches (`${streamId}:${i}:${ts}`). */
  trancheKey: text("tranche_key").notNull(),
  /** USD value at unlock time. Null until priced, or when no price exists. */
  usdValueAtUnlock: numeric("usd_value_at_unlock"),
  /** Pricing confidence:
   *    "exact"   — CoinGecko historical at the exact day
   *    "nearest" — nearest available price within the ±7-day fallback window
   *    "missing" — no historical price; usdValueAtUnlock is null (needs input)
   *    "manual"  — user-entered fair market value
   */
  priceConfidence: text("price_confidence").notNull().default("missing"),
  /** True when usdValueAtUnlock was hand-entered by the user (FMV override).
   *  Pricing passes must NOT overwrite these rows. */
  manualPrice: boolean("manual_price").default(false).notNull(),
  /** When this row was generated by the unlock-events pipeline. */
