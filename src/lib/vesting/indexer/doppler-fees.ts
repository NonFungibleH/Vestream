...
//
// Per window, three events on the locker:
//   Lock(poolId, (beneficiary, shares)[], unlockDate)  → one row per beneficiary
//   UpdateBeneficiary(poolId, old, new)                → move the row
//   Unlock(poolId, recipient)                          → flag is_unlocked
// plus one `streams(poolId)` read per new pool for the currencies, recipient,
// start date and lock duration. The claimable numbers are NOT stored here;
...
const UNLOCK_EVENT = FEE_LOCKER_ABI[1];
const UPDATE_EVENT = FEE_LOCKER_ABI[2];
...
function makeIndexer(chainId: SupportedChainId): Indexer {
  ...
  return {
    ...
    async scanWindow(client: PublicClient, fromBlock: bigint, toBlock: bigint) {
      const [locks, updates, unlocks] = await Promise.all([
        client.getLogs({ address: locker, event: LOCK_EVENT,   fromBlock, toBlock }),
        client.getLogs({ address: locker, event: UPDATE_EVENT, fromBlock, toBlock }),
        client.getLogs({ address: locker, event: UNLOCK_EVENT, fromBlock, toBlock }),
      ]);
      const eventCount = locks.length + updates.length + unlocks.length;
        ...
        try {
          await db.execute(sql`
            UPDATE doppler_fee_streams SET beneficiary = ${to}
             WHERE chain_id = ${chainId} AND pool_id = ${poolId} AND beneficiary = ${from}
