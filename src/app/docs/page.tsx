            ...
            {g.pages.map((p) => (
              <Link key={p.slug} href={`/docs/${p.slug}`}
                ...
                <div className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(37,99,235,0.08)", color: "#2563eb" }}>
                    <DocsIcon name={p.icon} size={18} />
                  </span>
                  <div className="min-w-0">
                    <p className="font-bold text-[15px] mb-0.5" style={{ color: "#0f172a" }}>{p.title}</p>
