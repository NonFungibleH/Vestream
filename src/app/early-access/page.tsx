  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#F5F5F3" }}>
      <div className="w-full max-w-sm rounded-3xl px-8 py-10 flex flex-col items-center text-center"
        ...
        {betaFull ? (
          <>
            <h1 className="text-xl font-bold mb-1" style={{ color: "#1A1D20", letterSpacing: "-0.02em" }}>Beta is Full</h1>
            <p className="text-sm mb-8" style={{ color: "#8B8E92" }}>
          ...
          <>
            ...
            <div className="flex items-center gap-2 mb-6">
              {(["code", "email", "otp"] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  ...
                  {i < 2 && <div className="w-8 h-px" style={{ background: "rgba(21,23,26,0.10)" }} />}
                </div>
              ))}
            </div>
            ...
            {slotsLeft !== null && slotsLeft <= 20 && step === "code" ? (
              <p className="text-xs font-semibold mt-2 mb-6" style={{ color: "#F0992E" }}>
            ...
            {step === "email" && (
              <form onSubmit={handleEmail} className="w-full flex flex-col gap-3">
                ...
                <button type="button" onClick={() => { setStep("code"); setError(""); }}
                  className="text-xs" style={{ color: "#B8BABD" }}>← Back</button>
              </form>
            )}
            ...
            {step === "otp" && (
              <form onSubmit={handleOtp} className="w-full flex flex-col gap-3">
                ...
                <button type="button" onClick={() => { setStep("email"); setOtp(""); setError(""); }}
                  className="text-xs" style={{ color: "#B8BABD" }}>← Resend to different email</button>
              </form>
            )}

            <p className="text-xs mt-6" style={{ color: "#B8BABD" }}>
              Don&apos;t have an access code?{" "}
              <Link href="/" className="underline" style={{ color: "#8B8E92" }}>Join the waitlist</Link>
            </p>
          </>
        )}
      </div>
