...
| `/api/claims/export` | GET: tax-ready CSV (Koinly / CoinTracker / TurboTax / generic), filters `since`/`until`/`tokenAddress`. `?basis=claim\|unlock` (defaults to the user's stored `taxBasis`) selects which income table to export from — claim rows or unlock rows, adapted to a common `TaxExportRow` shape. Persists a copy to `taxReportFiles`. |
| `/api/tax/basis` | GET/PUT the user's `taxBasis` (`claim`\|`unlock`). Dual-auth (web cookie OR mobile Bearer). |
| `/api/tax/unlock-events` | GET: the dashboard tax table's data — one row per unlock tranche with BOTH bases (unlock + claim) merged per tranche, plus the user's stored basis. |
| `/api/tax/unlock-events/[id]/price` | PUT `{ usd }`: manual fair-market-value override for a `missing`-priced unlock event (pre-liquid tokens). Guards: row must belong to the user AND still be `missing`. |
...
| `/api/cron/generate-unlocks` | Cron (daily 07:30 UTC, after ingest-claims): for each paid user, reads cached streams → `generateUnlockEventsForUser` (computes discrete unlock tranches) → `priceUnlockEvents` (prices `missing` rows). Synchronous/awaited (Vercel kills `after()`), per-user try/catch, `CRON_SECRET`-gated. `?userId=` scopes to one user. Idempotent. |
...
| `claimEvents` | Per-claim income rows for the tax feature. One row per on-chain withdrawal/claim: `(userId, streamId, protocol, chainId, tokenAddress, tokenSymbol, amount, claimedAt, usdValueAtClaim)`. Populated by the per-protocol claim ingestors (`src/lib/vesting/ingestors/*-claims.ts`) via the `ingest-claims` cron. `usdValueAtClaim` is priced at receipt via `getHistoricalPrice`. This is the **claim-basis income** half of the Tax Reports tool. |
| `vestingUnlockEvents` | **Unlock-basis (accrual) income** — counterpart to `claimEvents` for jurisdictions that tax at unlock, not claim. One row per computed unlock tranche: `(userId, streamId, protocol, chainId, recipient, tokenAddress, tokenSymbol, tokenDecimals, amount, unlockTime, trancheKey, usdValueAtUnlock, priceConfidence, manualPrice)`. **Unlocks are NOT on-chain events** — rows are COMPUTED from each stream's vesting schedule by `computeUnlockTranches` (`src/lib/vesting/unlock-schedule.ts`; discrete tranches only — cliff/stepped/instant; linear/continuous deferred to a later phase), then priced at the unlock timestamp via `getHistoricalPrice`. Generated + priced by the `generate-unlocks` cron (logic in `src/lib/vesting/unlock-events.ts`). Unique on `(userId, trancheKey)` so regeneration never double-inserts. `priceConfidence` ∈ `exact\|nearest\|missing\|manual`; `missing` rows show "needs your input" in the UI and accept a manual FMV (sets `manualPrice=true`; pricing passes skip those rows). |
...
`users.taxBasis` (`"claim" | "unlock"`, default `"claim"`) — added 2026-07-05. Which basis the user reports vesting income on. Claim stays the default; unlock is opt-in via the tax dashboard toggle. Read/set via `/api/tax/basis`; consumed by `/api/claims/export` (a report is emitted on exactly ONE basis, never both, so income can't double-count). Type guard in `src/lib/tax/tax-basis.ts`.
...
| Team Finance | `team-finance` | ETH, BSC, Polygon, Sepolia | Squid GraphQL (different stack than The Graph) | **Base dropped 2026-07-06** — TF's Squid indexes ZERO Base vestings (verified against the endpoint: ETH 1,075 / BSC 1,028 / Base 0) and the claims Squid has no Base either, so we can't get correct withdrawn amounts even via the per-wallet REST fallback. Removed Base from the adapter `supportedChainIds`, protocol `chainIds`, and the seeder job rather than show incomplete/incorrect data or a permanent "$0 on Base". Re-add all three if TF starts indexing Base upstream. **LIVE again (2026-06-30) — `disabled: false`.** Paused May 2026 → June 2026 (purged cache + TVL rows; legal not yet agreed); re-enabled with go-live permission. The flag flip + a deep-seed repopulate the cache/TVL rows; it's back on every user-facing surface (homepage protocol strip, /protocols, search). Adapter/walker/ingestor code was kept intact throughout the pause. TVL is self-indexed via the Squid `vestingFactoryVestings` walk — watch the per-token $200M high-confidence ceiling (Team Finance's thin-pair memecoins triggered the fake-headline failure mode pre-fix; see TVL Methodology). |
...
- **Headline TVL and the Explorer table are DIFFERENT populations for merkle-distributed protocols — they never reconcile, and that's correct.** The TVL walker enumerates the FULL on-chain factory (`vestingFactoryVestings` for Team Finance) — every contract, regardless of whether any wallet has been discovered. The Explorer/rollup table (`token_vesting_rollups`, derived from `vesting_streams_cache`) only contains streams for wallets we've actually indexed/claimed — a PARTIAL subset. For a 100%-merkle-distributed protocol like Team Finance, almost no per-wallet rows exist, so the Explorer export sums to a tiny number (~$204K on BNB) while the genuine headline TVL is ~$146M (real locks: DUCKY, SMILEK, VBT, FAYA, PRDT — high-liquidity BNB memecoins, generously but legitimately valued and capped by the per-token ceiling above). **Do not "fix" this gap by forcing the two to match** — they measure different things. If asked "where does the TVL come from, it's not in the table", the answer is: the factory walk, not the wallet cache. `scripts/audit-tf-tvl.ts` reproduces the live walker + pricing breakdown for verification.
...
- **Don't surface a `disabled` protocol.** `listProtocols()` excludes disabled protocols by default, the seeder skips them, and `dbcache` filters their rows out of reads. (Team Finance was disabled May–June 2026 but went **live again 2026-06-30** — it is no longer an example of a paused protocol. Keep any *future* paused protocol off every user-facing surface.)
- **NEVER lowercase a Solana address.** Solana mints/wallets are base58 and **case-sensitive** (base58 excludes `0 O I l`); lowercasing corrupts them and breaks every link/lookup. Use `normaliseAddress` from `src/lib/address-validation.ts` — it lowercases EVM (`0x…`) addresses but preserves Solana/other ecosystems verbatim. Any `count(distinct token_address)` / `count(distinct recipient)` in SQL must be case-insensitive **for EVM only**: `count(distinct case when token_address like '0x%' then lower(token_address) else token_address end)`. Plain `lower(...)` over-collapses distinct Solana mints and undercounts. (Bit us: TF token count + broken Solana token pages, fixed June 2026.)
- **`after()` background work is UNRELIABLE on Vercel — cron and ISR must do their work synchronously.** Vercel kills the function once the response is flushed, so `after(() => …)` frequently never runs (the `refresh-rollups` cron silently froze for weeks this way). Cron handlers invoked directly by Vercel bypass Cloudflare's 100s gateway cap and run synchronously up to `maxDuration` (300s proven by tvl-snapshot ~191s) — `await` the work, wrap each step in try/catch, don't fire-and-forget. Keep an `?background=true` escape hatch only for manual Cloudflare-fronted calls that would hit the 100s cap. The 4 derived tables (`token_vesting_rollups`, `protocol_summaries`, `status_summary`, `protocol_tvl_snapshots`) are each refreshed by SEPARATE crons — a stale Explorer/status page usually means one of those crons stopped, not a render bug.
- **Never claim "CI is green" until a FULL `npm run build` completes locally.** Static generation (`/token/[chainId]/[address]`, protocol pages) runs at build time with no reliable DB — build-time DB helpers MUST short-circuit on `process.env.NEXT_PHASE === "phase-production-build"` (return null/empty) or prerender throws. Killing the build early "to save time" hides exactly the prerender errors that break CI. Let it finish.
- **"Active" ≠ "upcoming unlock" — but the protocol hero now RECONCILES to the live scope (2026-07).** Historically the hero's "Active now" counted every `withdrawnAmount < totalAmount` stream (past-due-but-unclaimed included), which contradicted the calendar's future-only "upcoming" count and confused users. `refreshProtocolSummaries` now computes the hero's Active/Tokens/Recipients on the **live scope** — `is_fully_vested=false AND (end_time>now OR end_time≤0/null)`, the SAME population the /unlocks calendar groups — so they agree across surfaces. The past-due-unclaimed tail didn't disappear: it's surfaced as its own **"Unclaimed"** hero stat (`unclaimed_streams` = fully-vested-but-`withdrawn<total`). "Streams indexed" stays all-time (the footprint) and "Chains covered" stays the integration count (`meta.chainIds`), both deliberately NOT reconciled. Continuous-stream protocols (llamapay, sablier-flow) store `is_fully_vested=true`, so they'd zero under the live filter — their Active/Tokens/Recipients fall back to the all-time counts (keyed off `PROTOCOL_DEFAULT_CATEGORY[...] === "stream"`). If you change the hero scope, change it in BOTH `refreshProtocolSummaries` and the `computeProtocolStatsFromCache` bootstrap fallback.
