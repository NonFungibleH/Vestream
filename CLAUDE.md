...
- **"Active" ≠ "upcoming unlock".** A stream is **active** when `withdrawnAmount < totalAmount` (or `withdrawnAmount null AND !is_fully_vested`) — this INCLUDES past-due-but-unclaimed locks. The unlocks/calendar surfaces show only **future** unlocks (`is_fully_vested=false AND end_time>now`), then group them by token+date. So a protocol hero showing "146 active streams" and its calendar showing "26 unlocks" is not a contradiction — 146 active = future-unlock + past-due-unclaimed; the 26 is future-only, grouped. Different scopes by design; don't reconcile them.
